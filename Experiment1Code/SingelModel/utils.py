...
def save_checkpoint(epoch, model, filename="my_checkpoint.pth.tar"):
    print(f"=> Saving checkpoint of epoch {epoch}")
    filepath = f"model_epoch_{epoch}.pth"
    torch.save(model.state_dict(), filepath)
    os.chmod(filepath, 0o644)


def load_checkpoint(model):
    list_of_files = [fname for fname in glob.glob("./model_epoch_*")]
    if not list_of_files:
        print("No checkpoint found.")
        return
    latest_epoch_file = max(list_of_files, key=os.path.getctime)
    print(f"=> Loading checkpoint from: {latest_epoch_file}")
    state_dict = torch.load(latest_epoch_file)
...
def get_loaders(
    ...
    # Split the training dataset based on client_id
    dataset_length_train = len(train_ds)
    per_client_length_train = dataset_length_train // total_clients
    start_idx_train = (client_id - 1) * per_client_length_train
    end_idx_train = start_idx_train + per_client_length_train
