...
class FlowerClient(fl.client.NumPyClient):
    def __init__(self, cid, model, train_loader, val_loader, shift_size=5, initial_samples=150, initial_epochs=5, subsequent_epochs=2,subsequent_samples=25):
        self.cid = cid
        self.model = model
        ...
        self.subsequent_samples = subsequent_samples  # Set subsequent_samples as an instance variable
    ...
    def fit(self, parameters, config):
        ...
        if self.first_round:
            ...
            self.shift_size = self.initial_samples  # Set the shift size to initial samples for the first round
            self.first_round = False
        else:
            epochs = self.subsequent_epochs
            self.shift_size += 5  # Incrementing the shift_size by 5 samples.
        
        # Update the DataLoader for each round directly using get_loaders
        self.train_loader, self.val_loader, self.test_loader  = get_loaders(
            ...
            subsequent_samples=self.subsequent_samples,
            round_num=self.aggregation_round,
        )
        
        train(self.model, self.train_loader, self.val_loader, epochs=epochs)
    ...
    def evaluate(self, parameters, config):
        ...
        loss, num_examples, results = check_accuracy(self.val_loader, self.model, device=DEVICE)
        save_predictions_as_imgs(self.val_loader, self.model, folder="saved_images/", device=DEVICE)
        save_evaluation_results(results, filename="eval_results_round.csv", client_id=CLIENT_ID, round_number=self.aggregation_round)
